...
using uThreads::io::IOPoller;
using ::epoll_event;

IOPoller::IOPoller(IOHandler &i) : ioh(i) {
    // kernel version >= 2.9
    epoll_fd = epoll_create1(EPOLL_CLOEXEC);
    // TODO(saman): Make sure this is backward compatible with kernel < 2.9
    if (epoll_fd == -1) {
 ...
 * IOHandler, it is important to make sure this does not add extra overhead.
 */
...
ssize_t IOPoller::_Poll(int timeout) {
    ...
    // TODO(saman): dedicated thread always blocks but others should not
