...
void IOHandler::open(PollData &pd) {
    ...
    // TODO(saman): use a mutex instead?
    if (!__atomic_compare_exchange_n(&pd.opened, &expected, true, false,
    ...
    if (res != 0) {
        ...
        // TODO(saman): this should be changed to an exception
        exit(EXIT_FAILURE);
    }
    // TODO(saman): handle epoll errors
}
...
void IOHandler::block(PollData &pd, bool isRead) {
    ...
    kThread::currentKT->currentUT->suspend(
            (funcvoid2_t) IOHandler::postSwitchFunc,
            (void *) &pd);
    // ask for immediate suspension so the possible closing/notifications
    // do not get lost when epoll returns this ut will be back
    // on readyQueue and pick up from here
}
...
void IOHandler::postSwitchFunc(void *ut, void *args) {
    ...
    while (true) {
        ...
        if (__atomic_compare_exchange_n(utp, &expected, utold, false,
...
int IOHandler::close(PollData &pd) {
    ...
    if (!__atomic_compare_exchange_n(&pd.closing, &expected, true, false,
                                     __ATOMIC_ACQUIRE, __ATOMIC_RELAXED))
        return -1;
...
bool IOHandler::unblock(PollData &pd, bool isRead) {
    ...
    while (true) {
        ...
        if (__atomic_compare_exchange_n(utp, &old, utnew, false,
                                        __ATOMIC_RELAXED, __ATOMIC_RELAXED)) {
            if (old > POLL_WAIT) {
#ifdef NPOLLBULKPUSH
                old->resume();
