...
uThread *uThread::create(size_t ss, bool joinable) {
    ...
    if (ut == nullptr) {
        ...
        vaddr mem = uThread::createStack(ss);
...
void uThread::migrate(Cluster *cluster) {
    ...
    // no need to migrate
    if (slowpath(kThread::currentkThread()->localCluster == cluster))
        return;
...
void uThread::resume() {
    if (fastpath(
        ...
        // Put uThread back on ReadyQueue
        Scheduler::schedule(this, *homekThread);
    }
}
