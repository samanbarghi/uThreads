...
#include "KOS/runtime/Scheduler.h"
...
private:
    ...
    void removeMany(IntrusiveList<Thread>& nqueue, mword numkt){
...
public:
    ...
    void tryPopMany(IntrusiveList<Thread> &nqueue, mword numkt) {//Try to pop ReadyQueueSize/#kThreads in cluster from the ready Queue
        std::unique_lock<std::mutex> mlock(mtx, std::try_to_lock);
        if(!mlock.owns_lock() || size == 0) return; // There is no uThreads
        removeMany(nqueue, numkt);
    }

    void popMany(IntrusiveList<Thread> &nqueue, mword numkt) {//Pop with condition variable
...
class Cluster : public BaseScheduler {
    friend class kThread;
private:
...
public:
    ...
    void tryGetWorks(IntrusiveList<Thread>&);//Get as many uThreads as possible from the readyQueue and move them to local queue
    void getWork(IntrusiveList<Thread>&);//Get a unit of work or if not available sleep till there is one
